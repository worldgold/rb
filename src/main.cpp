...
#if defined(NDEBUG)
# error "Robocoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x3eadb18a8eef17b76fa0bc62ba7550b560c4a62b687754887cdda122ae39d4c5");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Robocoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Robocoin Signed Message:\n";
...
{
    // Robocoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 84000); // Robocoin: 84k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 2 * 60; // Robocoin: 2 minutes
static const int64 nTargetSpacing = 2 * 10; // Robocoin: 20 seconds
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Robocoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Robocoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        txNew.vout[0].nValue = 60000 * COIN;
        ...
        assert(block.hashMerkleRoot == uint256("0xc066b7d9fab12a14b6c6d6021625df004e8d5e8a6e900d80fd6e89f00c32ef34"));
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // Robocoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("RobocoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("RobocoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static RobocoinMiner(CWallet *pwallet)
{
    printf("RobocoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("robocoin-miner");
    ...
    try { loop {
        ...
        printf("Running RobocoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("RobocoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&RobocoinMiner, pwallet));
}
